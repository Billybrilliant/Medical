...
export default class Appointment extends Component {
  ...
  handleSelect = () => {
    ...
    return axios({
      ...
      for (var i = 0; i < data.length; i++) {
        jsx.push(<option value={data[i].dname}>{data[i].dname}</option>);
      }
      // console.log(jsx);
      this.setState({
        doctorList: jsx
  ...
  componentWillMount() {
    axios({
      ...
      for (var i in data) {
        $('#categoryOne').append(
          `<option value=${data[i].local_name}>${data[i].local_name}</option>`
        );
      }
    });
    axios({
      ...
      for (var i in data) {
        $('#categoryTwo').append(
          `<option value=${data[i].hname}>${data[i].hname}</option>`
        );
      }
    ...
    axios({
      ...
      for (var i in data) {
        $('#categoryThree').append(
          `<option value=${data[i].section}>${data[i].section}</option>`
        );
      }
    });
  }
